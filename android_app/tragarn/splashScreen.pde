...
pushMatrix();
  ...
  //tint(255,lFade);
  image(logosplash, 40*widthStretch, 400*heightStretch, 514*widthStretch, 242*heightStretch);
  //lFade = lFade + 8;
  ...
  for ( int is = 0; is < nodeLib.length; is++) {
    ...
    if ( nodeS[is] >= 1.08 ) { nodeS[is] = 1.01; }
      kurbits(nodeLib[is][0],nodeLib[is][1],nodeRs[is],nodeS[is]*2,nodeC[is],cyanpas, #ffea00);
